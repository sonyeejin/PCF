...
const app = express();
const PORT = 4001;                            // Site B 서버 포트
const PCF_BASE_URL = 'http://localhost:3000'; // PCF 백엔드 주소
const SITE_DOMAIN = 'b.com';                  // 보고서 기준 domain
...
app.get('/', (req, res) => {
  res.send(`
    <html>
      <head><title>Site B - Home</title></head>
      <body>
...
app.get('/login', (req, res) => {
  res.send(`
    <html>
      <head><title>Site B Login</title></head>
      <body>
        <h1>Site B Login</h1>
...
app.post('/login', async (req, res) => {
  ...
  console.log('[Site-B] Call /evaluate_login (headers-only mode):', {
    user_token,
    domain,
    login_ip
  });
  ...
  console.log('[Site-B] PCF headers from backend:', {
    login_event_id,
    domain_salt,
    run_sandbox,
  });
  ...
  if (!login_event_id || domain_salt === null || run_sandbox === null) {
    console.error('[Site-B] Missing required PCF headers from backend');
...
app.post('/pcf_result', (req, res) => {
  ...
  console.log('[Site-B] PCF 최종 결과 수신:', {
    login_event_id,
    risk_score,
    security_flags,
    user_token,
...
app.listen(PORT, () => {
  console.log(`Site B server running at http://localhost:${PORT}`);
});
